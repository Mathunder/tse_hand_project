...
//    GLfloat color[3] = {0.7f, 0.7f, 0.7f};
    ...
    //Colonne 1
    cube->drawCube(-7, 0, 0.f, color, false);

    //Colonne 2
    for(int i=-2; i<=1; i++)
        cube->drawCube(-6, i, 0.f, color, false);

    //Colone 3
    for(int i=-4; i<=-1; i++)
        cube->drawCube(-5, i, 0.f, color, false);

    //Colonne 4 & 5
    for(int j=-4; j<=-3; j++) {
        for(int i=-6; i<=7; i++)
            cube->drawCube(j, i, 0.f, color, false);
    }
    cube->drawCube(-3, -7, 0.f, color, false);

    //Colonne 6
    for(int i=-7; i<=3; i++)
        cube->drawCube(-2, i, 0.f, color, false);
    ...
    //Colonne 9
    for(int i=-7; i<=3; i++)
        cube->drawCube(1, i, 0.f, color, false);
    ...
    //Colonne 12
    for(int i=-7; i<=3; i++)
        cube->drawCube(4, i, 0.f, color, false);
...
void Hand::drawThumb(boolean is_drawn) {
    if(is_drawn) {
        ...
        for(int j=1; j<=2; j++) {
            for(int i=-8; i<=-7; i++)
                cube->drawCube(i, j, 0.f, color, false);
        }
...
//        cube->drawCube(-7, -1, 0.f, black);
        ...
        cube->drawCube(-7, -1, 0.f, color, false);
...
void Hand::drawLittleFinger(boolean is_drawn) {
    if(is_drawn) {
        ...
        cube->drawCube(6, 6, 0.f, color, false);
        // Contour du doigt
//        cube->drawCube(4, 8, 0.f, black);
